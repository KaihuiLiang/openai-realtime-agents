...
export default function RowActions({ id }: { id: string }) {
  ...
  const onDelete = async () => {
    if (!confirm('Delete this participant? Assignments may also be removed.')) return;
    setLoading(true);
    try {
      const res = await fetch(`/api/backend/participants/${id}`, { method: 'DELETE' });
      if (!res.ok) {
      ...
      alert(e.message || 'Failed to delete participant');
    } finally {
      setLoading(false);
    }
  };
